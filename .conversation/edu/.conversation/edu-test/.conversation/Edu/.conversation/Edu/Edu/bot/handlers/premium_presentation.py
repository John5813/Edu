...
async def premium_ppt_got_count(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    # Slayd soni foydalanuvchining tanlovi bo'lib qoladi; darajani AI belgilaydi.
    level = 2
    try:
        from services.premium_presentation.llm_client import plan_presentation
        plan = await asyncio.get_event_loop().run_in_executor(
            None, plan_presentation, topic, preferences, slide_count
        )
        level = max(1, min(3, int(plan.get("level", 2))))
    except Exception:
        logger.warning("AI daraja tanlashda xato, professional default ishlatiladi")
    level_label = LEVEL_LABELS.get(level, {}).get(lang, "")
