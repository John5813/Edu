...
class AIService:
    ...
    def __init__(self):
        api_key = (
            os.environ.get("AI_INTEGRATIONS_OPENROUTER_API_KEY")
...
async def generate_test_questions_from_source(source_text: str, count: int, language: str) -> list:
    """Turn a selected range of existing tests into Telegram quiz questions."""
    service = get_ai_service()
    lang_name = {"uz": "o'zbek", "ru": "русский", "en": "English"}.get(language, "o'zbek")
    prompt = (
        f"You are an expert teacher. Convert the following {count} existing test questions into quiz questions.\n"
        f"Respond ONLY in {lang_name} language. Preserve the meaning and facts from the source; do not invent "
        "answers that are not supported by it. Each question must have exactly 4 options and one correct answer.\n"
        "Return ONLY a valid JSON array (no markdown, no explanation) with exactly this structure:\n"
        '[{"question":"Savol matni?","options":["A variant","B variant","C variant","D variant"],'
        '"correct_index":0,"explanation":"Qisqa izoh"}]\n'
        "correct_index is 0-based (0=A, 1=B, 2=C, 3=D).\n\n"
        "SOURCE TESTS:\n"
        f"{source_text[:180000]}"
    )
    try:
        response = await service._make_request(
            messages=[{"role": "user", "content": prompt}],
        ...
        if isinstance(questions, list):
            ...
            for question in questions:
                ...
                if question.get("question") and len(options) == 4 and isinstance(correct_index, int) and 0 <= correct_index < 4:
