...
def get_course_work_page_keyboard(language: str = "uz") -> InlineKeyboardMarkup:
    ...
    # Kurs ishi qaysi mavzuda bo'lmasin uch bobdan iborat — varaq soni
    # ortganda bob qo'shilmaydi, har bobning hajmi kengayadi.
    options = (("15-20", 10000), ("20-25", 15000), ("25-30", 20000),
               ("30-35", 25000), ("40-50", 30000), ("50-60", 35000))
    if language == "ru":
        pattern = "{pages} стр (3 главы) - {price} сум"
    elif language == "en":
        pattern = "{pages} pages (3 chapters) - {price} som"
    else:
        pattern = "{pages} varoq (3 bob) - {price} so'm"

    for pages, price in options:
        low, high = pages.split("-")
        keyboard.add(InlineKeyboardButton(
            text=pattern.format(pages=pages, price=price),
            callback_data=f"cw_pages_{low}_{high}_3",
        ))
