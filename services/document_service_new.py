...
class DocumentService:
    ...
    async def create_new_presentation_with_template(self, topic: str, content: Dict, author: str, template_name: str = "classic") -> str:
        """Create presentation with NEW SYSTEM using templates and DALL-E images"""
        try:
            logger.info(f"Creating NEW presentation with template: {template_name}")
            
            # Import template service
            from services.presentation_templates import PresentationTemplates
            ...
            for i, slide_data in enumerate(slides):
                slide_title = slide_data.get('title', f'Slayd {i+1}')
                slide_content = slide_data.get('content', '')
                layout_type = slide_data.get('layout_type', 'bullet_points')
                slide_number = slide_data.get('slide_number', i+1)
                
                logger.info(f"Creating slide {slide_number}: {layout_type} - {slide_title}")
                ...
                template_colors = template_service.apply_template_to_slide(slide, template_name)
                text_color = template_colors['text']
                title_color = template_colors['title']
                
                if layout_type == "title":
                    await self._create_title_slide(slide, slide_title, slide_content, author, title_color, text_color)
                elif layout_type == "bullet_points":
                    await self._create_bullet_slide(slide, slide_title, slide_content, title_color, text_color)
                elif layout_type == "text_with_image":
                    await self._create_text_image_slide(slide, slide_title, slide_content, slide_number, title_color, text_color)
                elif layout_type == "three_column":
                    await self._create_three_column_slide(slide, slide_title, slide_content, title_color, text_color)
                else:
            ...
            filename = f"webapp_presentation_{timestamp}.pptx"
            file_path = os.path.join(self.documents_dir, filename)
            
            prs.save(file_path)
            logger.info(f"NEW template presentation saved: {file_path}")
            return file_path
            
        except Exception as e:
            logger.error(f"Error creating NEW template presentation: {e}")
            raise
    
    async def _create_title_slide(self, slide, title: str, content: str, author: str, title_color, text_color):
        """Create title slide"""
        try:
            # Title
            title_shape = slide.shapes.add_textbox(
                PptxInches(1), PptxInches(2), PptxInches(8), PptxInches(2)
            )
            ...
            author_shape = slide.shapes.add_textbox(
                PptxInches(1), PptxInches(5), PptxInches(8), PptxInches(1)
            )
            author_frame = author_shape.text_frame
            author_frame.text = f"Tayyorladi: {author}"
            author_frame.paragraphs[0].font.size = PptxPt(24)
    ...
    async def _create_bullet_slide(self, slide, title: str, content: str, title_color, text_color):
        ...
        try:
            ...
            content_shape = slide.shapes.add_textbox(
                PptxInches(1), PptxInches(1.5), PptxInches(8), PptxInches(5.5)
            )
            content_frame = content_shape.text_frame
            content_frame.text = content
            content_frame.paragraphs[0].font.size = PptxPt(18)
            content_frame.paragraphs[0].font.color.rgb = text_color
            
        except Exception as e:
            logger.error(f"Error creating bullet slide: {e}")
    
    async def _create_text_image_slide(self, slide, title: str, content: str, slide_number: int, title_color, text_color):
        """Create text with DALL-E image slide"""
        try:
            # Title
            title_shape = slide.shapes.add_textbox(
                PptxInches(1), PptxInches(0.5), PptxInches(8), PptxInches(1)
            ...
            text_shape = slide.shapes.add_textbox(
                PptxInches(1), PptxInches(1.5), PptxInches(4.5), PptxInches(5.5)
            )
            text_frame = text_shape.text_frame
            text_frame.text = content
            text_frame.paragraphs[0].font.size = PptxPt(18)
            text_frame.paragraphs[0].font.color.rgb = text_color
            
            # Generate and add DALL-E image (right side)
            try:
                image_url = await self.ai_service.generate_dalle_image(content, title)
                if image_url:
                    image_path = await self.ai_service.download_image(image_url, f"dalle_slide_{slide_number}.png")
                    if image_path and os.path.exists(image_path):
                        slide.shapes.add_picture(
                            image_path, 
                            PptxInches(5.5), PptxInches(1.5), 
                            PptxInches(3.5), PptxInches(4.5)
                        )
                        logger.info(f"Added DALL-E image to slide {slide_number}")
            except Exception as img_e:
                logger.error(f"Failed to add DALL-E image to slide {slide_number}: {img_e}")
                
        except Exception as e:
            logger.error(f"Error creating text+image slide: {e}")
    
    async def _create_three_column_slide(self, slide, title: str, content: str, title_color, text_color):
        ...
        try:
            ...
            if "|||" in content:
                parts = content.split("|||")
                if len(parts) >= 6:  # 3 headers + 3 contents
                    # Column 1
                    col1_shape = slide.shapes.add_textbox(
                        PptxInches(0.5), PptxInches(1.5), PptxInches(2.8), PptxInches(5)
                    )
                    col1_frame = col1_shape.text_frame
                    col1_frame.text = f"{parts[0]}\n\n{parts[1]}"
                    col1_frame.paragraphs[0].font.size = PptxPt(20)
                    col1_frame.paragraphs[0].font.bold = True
                    col1_frame.paragraphs[0].font.color.rgb = title_color
                    
                    # Column 2
                    col2_shape = slide.shapes.add_textbox(
                        PptxInches(3.6), PptxInches(1.5), PptxInches(2.8), PptxInches(5)
                    )
                    col2_frame = col2_shape.text_frame
                    col2_frame.text = f"{parts[2]}\n\n{parts[3]}"
                    col2_frame.paragraphs[0].font.size = PptxPt(20)
                    col2_frame.paragraphs[0].font.bold = True
                    col2_frame.paragraphs[0].font.color.rgb = title_color
                    
                    # Column 3
                    col3_shape = slide.shapes.add_textbox(
                        PptxInches(6.7), PptxInches(1.5), PptxInches(2.8), PptxInches(5)
                    )
                    col3_frame = col3_shape.text_frame
                    col3_frame.text = f"{parts[4]}\n\n{parts[5]}"
                    col3_frame.paragraphs[0].font.size = PptxPt(20)
                    col3_frame.paragraphs[0].font.bold = True
                    col3_frame.paragraphs[0].font.color.rgb = title_color
            else:
            ...
            logger.error(f"Error creating three column slide: {e}")
    
    async def create_new_presentation_system(self, topic: str, content: Dict, author: str) -> str:
        """Create presentation with NEW SYSTEM (without template)"""
        return await self.create_new_presentation_with_template(topic, content, author, "classic")
